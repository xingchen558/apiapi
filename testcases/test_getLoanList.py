...
cases = ww.get_cases('getLoanList')
@ddt
class TestList(unittest.TestCase):
    ...
    def test_list(self, case):
        ...
        if case.data != None:
            ...
            my_logger.debug(res.get_json()['data'][0])
            if res.get_json()['code'] == '10001':
                my_logger.debug("获取列表成功")
            elif res.get_json()['code'] == '20303':
                my_logger.debug("获取标列表失败")
            else:
                my_logger.debug("服务器异常")
    def tearDown(self):
        self.mysql.close()
        print("结束测试")
